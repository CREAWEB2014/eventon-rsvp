...
class evors_actionuser{
	...
	// event manager additions
		...
		function event_manager_row($event_id, $EPMV){
			if( evo_check_yn($EPMV,'evors_rsvp') ){
				echo "<a class='evoauem_additional_buttons load_rsvp_stats' data-eid='{$event_id}'>".evo_lang('View RSVP Stats')."</a>";
			}
		}
		...
		function event_manager_show_data($event_id){
			...
			?>
			<div id='evorsau_rsvp_section'>
				<h3 class="evoauem_section_subtitle"><?php evo_lang_e('Event');?>: <b><?php echo get_the_title($event_id);?></b></h3>
				<h3 class='evoauem_section_subtitle' style=''><?php evo_lang_e('RSVP Information & Stats');?></h3>	
				<div class="evoaursvp_data">
					
					<div id="evorsau_stats">
						<p class='caption'><?php evo_lang_e('Total Capacity');?></p>
						<p class='num'><?php

							$total = evo_var_val($EPMV,'evors_capacity_count');
							echo !empty($total)? $total:'na';

							$synced = $eventon_rs->frontend->functions->total_rsvp_counts($EPMV);
					...
					<?php 
						$RSVP_LIST = $eventon_rs->frontend->functions->GET_rsvp_list($event_id, 'all');
						...
						if($RSVP_LIST):
							...
							<div id='evorsau_attendee_list' class='evors_list <?php echo evo_settings_check_yn($evors_opt,'evorsau_check_guest')?'checkable ':'';?>evorsau_attendee_list'>	
								...
								<h4><?php evo_lang_e('Attendees');?></h4>
								...
								if(!empty($RSVP_LIST['m']) && count($RSVP_LIST['m'])>0){
									...
									echo "</ul>";
								}else{	echo "<p class='noone'>".evo_lang('No Attendees found.')."</p>";	}	
		...
		function each_attendee_data_row($_id, $rsvp, $text){
			...
			?>
			<li data-rsvpid='<?php echo $_id;?>'><em class='evorsadmin_rsvp' title='<?php _e('Click for more information','eventon');?>'><?php echo '#'.$_id;?></em><?php echo ' '. $rsvp['name'].' <i>('.$rsvp['email'].( $phone? ' PHONE:'.$phone:'').')</i>';?><span data-id='<?php echo $_id;?>' data-status='<?php echo $_status;?>' class='checkin <?php echo ($_status=='checked')? 'checked':null;?>'><?php echo $_status;?></span><span><?php echo $rsvp['count'];?></span>
				<?php 

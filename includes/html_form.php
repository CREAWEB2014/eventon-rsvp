...
<div id='evors_get_form'>
	...
	<div id='evorsvp_form' class=''>
		...
		<form class='evors_submission_form' method="POST" action="" enctype="multipart/form-data">
			...
			<input type="hidden" name='repeat_interval' value=''/>
		...
		<div class='submission_form form_section'>
			...
			<?php
				...
				$_name = isset($_REQUEST['nm'])? $_REQUEST['nm']: '';
				$_namel = isset($_REQUEST['lnm'])? $_REQUEST['lnm']: '';
			?>
			<div class="form_row">
				<input class='name input req' name='first_name' type="text" placeholder='<?php echo $_field_fname;?>' title='<?php echo $_field_fname;?>' data-passed='' value='<?php echo $_name;?>'/>
				<input class='name input' name='last_name' type="text" placeholder='<?php echo $_field_lname;?>' title='<?php echo $_field_lname;?>' data-passed='' value='<?php echo $_namel;?>'/>
			</div>
		
			<?php	
				$_field_email = $this->lang( 'evoRSL_009','Email Address');

				$_email = isset($_REQUEST['em'])? $_REQUEST['em']: '';
			?>
			<div class="form_row">
				<input class='regular input req' name='email' type="text" placeholder='<?php echo $_field_email;?>' title='<?php echo $_field_email;?>' data-passed='' value='<?php echo $_email;?>'/>
			</div>
		...
		<?php
			...
			if($active_fields && in_array('names', $active_fields)):
				$_field_names = $this->lang('evoRSL_010b','List Full Name of Other Guests');
		?>
				...
				for($x=1; $x <= $eventon_rs->frontend->addFields; $x++){
					...
					?>
						...
						<?php 
							if($FIELDTYPE=='text'):
						?>
							<p class='inputfield'><label for=""><?php echo $optRS['evors_addf'.$x.'_1'];?></label>
